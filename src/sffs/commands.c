...
static inline void InitFileSalt(u32 inode, const FileSystemTableEntry* fstEntry)
{
	...
	_fileSalt.Inode = inode;
	_fileSalt.Miscellaneous = fstEntry->SFFSGeneration;
	memset(_fileSalt.Unknown, 0, sizeof(_fileSalt.Unknown));
}
...
s32 Format(u32 userId, FSHandle* fileHandles, u32 fileHandleCount)
{
	...
	root->Mode.Fields.OwnerPermissions = 0;
	root->Mode.Fields.GroupPermissions = 1; /* ro */
	root->Mode.Fields.OtherPermissions = 1; /* ro */
	root->Attributes = 0;
...
s32 ReadDirectory(const u32 uid, const u16 gid, const char* path, char* files, u32* numberOfEntries)
{
	...
	{
		u32 childInode = i == 0 ? entry->StartCluster : entry->Sibling;
		if (childInode == SFFSErasedNode)
		{
			*numberOfEntries = i;
			return IPC_SUCCESS;
		}

		entry = GetFstEntry(superblock, childInode);
		if (!files)
			continue;

		strncpy(filename, entry->Name, MAX_FILE_SIZE);
...
s32 SeekFile(FSHandle* handle, s32 offset, SeekMode whence)
{
	...
	// Ensure position is cluster-aligned (must be a multiple of CLUSTER_SIZE)
	if ((newPosition & (CLUSTER_SIZE - 1)) != 0)
		return FS_EINVAL;
...
s32 ReadFile(FSHandle* handle, u8* data, u32 length)
{
	...
	if (handle == NULL || data == NULL || (length & (CLUSTER_SIZE - 1)) != 0 || length == 0)
		return FS_EINVAL;
	...
	u16 cluster = TraverseClusterChain(superblock, fstEntry->StartCluster, chainIndex);

	// Prepare per-file salt data for AES/HMAC operations
	InitFileSalt(inode, fstEntry);
...
s32 WriteFile(FSHandle* handle, const void* data, u32 length)
{
	...
	// Check if handle has write permission
